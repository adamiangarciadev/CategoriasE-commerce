// app.js — Gestor de Categorías y Asignación de Artículos (sin codcat1..4)
...
// ====== CATEGORÍAS ======
let categoriaRows = [];
let rutaToCodes = {};        // "Mujer > ..." -> [codeNivel1, codeNivel2, ...]
...
EL('fileCategorias').addEventListener('change', async (ev)=>{
  ...
  try{
    ...
    // Helper: arma el trail de códigos (root -> nodo) siguiendo padres
...
function buildRamasPorGenero(){
  ...
  // Uso las rutas disponibles (las que construimos arriba)
  Object.keys(rutaToCodes).forEach(ruta=>{
    const root = ruta.split('>')[0]?.trim() || '';
    if (/mujer/i.test(root)) sets.Mujer.add(ruta);
    else if (/hombre/i.test(root)) sets.Hombre.add(ruta);
    else if (/niñ|nene|nena|nino|niño|nina|kids|infantil/i.test(root)) sets.Niños.add(ruta);
  });
...
// Export mapeo (coma) — opcional
EL('btnExportArt').addEventListener('click', ()=>{
  const cols = ['codigo','descripcion','genero','rama'];
  const lines = [cols.join(',')].concat(
    articulos.filter(a=> a.ramaSeleccionada).map(a=> cols.map(c=> csvCell(c==='rama'? a.ramaSeleccionada : a[c])).join(','))
  );
  const blob = new Blob([lines.join('\n')], {type:'text/csv;charset=utf-8;'});
  const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download='articulos_ramas.csv'; a.click();
});

// ✅ Export asignación por nivel con ";"
// Usamos rutaToCodes[ruta] que ahora viene del árbol (padres) → lista de códigos por nivel
